...
public class EnemyWaveManager implements OutputData.Data {
	...
	public void addWave(ArrayList<Enemy> enemies, final long time) {
		timeOffset += time;
		waves.put(new Long(timeOffset), new EnemyWave(enemies));
	}

	public void update(final long lastFrameDeltaTimeMS, EffectManager effectManager, Player player, AudioPlayer audioPlayer) {
		checkNextWave(lastFrameDeltaTimeMS);
		...
		while (iter.hasNext()) {
			...
			if (enemy.isDead()) {
				final Vector2 effectPos = enemy.get2DPos().add(ProjectileManager.HEIGHT_OFFSET);
				effectManager.addEffect(
						new AnimatedParticle(600, -90.0f, resRet.getBmpDeathAnim(), effectPos, 6, 1, 72.0f));
				effectManager.addEffect(
						new AnimatedParticle(600,  90.0f, resRet.getBmpDeathAnim(), effectPos, 6, 1, 72.0f));
	...
	private void checkNextWave(final long lastFrameDeltaTimeMS) {
